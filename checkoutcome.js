		...
		init_view_items : function(Y, url, sesskey, cmid, checkoutcomeid,periodid) {			
		    ...
		    for (var i=0; i<selects.length; i++) {
				YE.addListener(selects[i], 'change', function (e) {
				    M.mod_checkoutcome.select_change(this, e);
				});
		    }
		    ...
		    for (var i=0; i<add_comments.length; i++) {
				YE.addListener(add_comments[i], 'click', function (e) {
				    M.mod_checkoutcome.add_comment_click(this, e);
				});
		    }
...
//			};
		    ...
		    for (var i=0; i<plus.length; i++) {
		    	YE.addListener(plus[i], 'click', function(e) {
		    	    ...
		    	    M.mod_checkoutcome.update_count_server(this);
		    	});		    	
		    }
		...
		init_view_items_teacher : function(Y, url, sesskey, cmid, checkoutcomeid, studentid, groupid, periodid) {			
		    ...
		    for (var i=0; i<selects.length; i++) {
				YE.addListener(selects[i], 'change', function (e) {
				    M.mod_checkoutcome.select_change_teacher(this, e);
				});
		    }
		    ...
		    for (var i=0; i<add_teacher_comments.length; i++) {
				YE.addListener(add_teacher_comments[i], 'click', function (e) {
				    M.mod_checkoutcome.add_teacher_comment_click(this, e);
				});
		    }
		...
		init_view_display : function(Y, url, sesskey, cmid, checkoutcomeid, displaycolor) {
			...
			YE.addListener(id_color, 'click', function (e) {
			    M.mod_checkoutcome.show_color_picker();
			});	
		...
		init_edit_items : function(Y, url, sesskey, cmid, checkoutcomeid, imgediturl, imgaddurl, imgdelurl) {			
			...
			var handleAddSubmit = function() {				
				linkurl = document.getElementById('input_link').value;
				// check url
				if (!M.mod_checkoutcome.isValidURL(linkurl)) {
					alert('URL is not valid');
					return;
				}
				itemid = document.getElementById('itemid_link').value;
				sname = document.getElementById('sname_link').value;
				//alert('linkurl : ' + linkurl + ' - itemid : ' + itemid);
				M.mod_checkoutcome.send_update_link(itemid,linkurl,sname,imgediturl,imgaddurl,imgdelurl);				
			};
			...
			var handleDelSubmit = function() {				
				itemid = document.getElementById('itemid_link').value;
				sname = document.getElementById('sname_link').value;
				M.mod_checkoutcome.send_delete_link(itemid,sname,imgaddurl);				
			};
			
			addLinkDialog = new YAHOO.widget.Dialog("dialog_linkurl", {
			    width: "600px", 
			    fixedcenter: true, 
		    ...
		    for (var i=0; i<imgs.length; i++) {
				YE.addListener(imgs[i], 'click', function (e) {
					...
					sname = id.substring(id.lastIndexOf("_") + 1);
					itemid = id.substring(id.indexOf("_") + 1, id.lastIndexOf("_"));
					// get input value if existing
					hreflink = YD.getAttribute(YD.get('resourcelink_'+itemid),'href');
					if (!hreflink) {
						hreflink = '';
					}
					// Set content
		    ...
		    deleteLinkDialog = new YAHOO.widget.Dialog("dialog_deletelink", {
			    width: "600px", 
			    fixedcenter: true, 
			    constraintoviewport: true, 
			    underlay: "shadow", 
		    ...
		    for (var i=0; i<plus.length; i++) {
		    	YE.addListener(plus[i], 'click', function(e) {
		    		...
		    		n++;
		    		this.nextSibling.value = n;			
		    	    M.mod_checkoutcome.update_countgoal_server(this);
		    	});		    	
		    }
		    var minus = YD.getElementsByClassName('counter_minus');
		    for (var i=0; i<minus.length; i++) {
...
//		    window.onunload =  function(e) {
//				M.mod_checkoutcome.send_update_countgoal_batch(true);
//		    };
		...
		show_color_picker : function() {
			...
			color_container = document.createElement('span');
...
//			});
		...
		update_grade_server: function(id, grade) {
		    ...
		    this.updategradetimeout = setTimeout(function() {
				M.mod_checkoutcome.send_update_grade_batch(false);
			}, 1000);
		},		
		...
		update_gradelist_teacher: function(id, grade) {
		    ...
		    this.updategradetimeout = setTimeout(function() {
				M.mod_checkoutcome.send_update_grade_teacher_batch(false);
			}, 1000);
		...
		update_countgoal_server: function(el) {
		    ...
		    this.updatecountgoaltimeout = setTimeout(function() {
				M.mod_checkoutcome.send_update_countgoal_batch(false);
			}, 1000);
		},
		...
		update_count_server: function(el) {
		    ...
		    this.updatecounttimeout = setTimeout(function() {
				M.mod_checkoutcome.send_update_count_batch(false);
			}, 1000);
		},
		...
		send_update_grade_batch: function(unload) {
			...
			var callback= {
			    success: function(o) {
				    ...
				    var YD = YAHOO.util.Dom;
				    ajaxLoader = YD.get('ajaxLoadPanel_c');
					body = YD.getAncestorByTagName(ajaxLoader,'body');			
					body.removeChild(ajaxLoader);
					
					now = new Date();
					date = M.mod_checkoutcome.get_formatted_date(now,'fr');
					for (var itemid in items) {				    
				    	sel = YD.get('selstu_' + itemid);
					...
					lastdate = YD.get('lastdatestudent');
		...
		send_update_grade_teacher_batch: function(unload) {
			...
			var callback= {
			    success: function(o) {
					...
					var now = new Date();
					var date = M.mod_checkoutcome.get_formatted_date(now,'fr');
					for (var itemid in items) {	
				    	sel = YD.get('seltea_' + itemid);
		...
		get_formatted_date: function(date,lang) {
			...
			hours = M.mod_checkoutcome.formatHM(hours);
			minutes = M.mod_checkoutcome.formatHM(minutes);
		...
		send_update_link: function(itemid,linkurl, sname, imgediturl, imgaddurl,imgdelurl) {    
		    ...
		    var callback= {
			   success: function(o) {
					var YD = YAHOO.util.Dom;					
					ajaxLoader = YD.get('ajaxLoadPanel_c');
					dialog = YD.get('dialog_linkurl_c');	
					body = YD.getAncestorByTagName(dialog,'body');					
					body.removeChild(ajaxLoader);
					...
					hreflink = YD.get('resourcelink_' + itemid);
					if (!hreflink) {
						tr = YD.getAncestorByTagName(img,'tr');
						td = YD.getChildren(tr)[2];
		 ...
		 // Send message to server
		    var callback= {
			   success: function(o) {
					...
					hreflink = YD.get('resourcelink_' + itemid);
					link_content = hreflink.innerHTML;
					tr = YD.getAncestorByTagName(img,'tr');
					td = YD.getChildren(tr)[2];

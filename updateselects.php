...
// This file is part of the Checkoutcome plugin for Moodle - http://moodle.org/
//
...
if (!has_capability('mod/checkoutcome:updateown', $context) && !has_capability('mod/checkoutcome:updateother', $context)) {
    echo get_string('error_update', 'checkoutcome'); // 'Error: you do not have permission to update this checkoutcome';
    die();
}
if (!confirm_sesskey()) {
    echo get_string('error_sesskey', 'checkoutcome'); // 'Error: invalid sesskey';
    die();
}
if (!$items || !is_array($items)) {
    echo get_string('error_itemlist', 'checkoutcome'); // 'Error: invalid (or missing) items list';
    die();
}
if (!empty($items)) {
    $chk = new checkoutcome_class($cm->id, $userid, $checkoutcome, $cm, $course, $studentid, null, $periodid);
    if ($studentid == 0) {
    	$chk->ajaxupdateselects($items);
    } else {
    	$chk->ajaxupdateteacherselects($items);
    }
}

echo get_string('OK', 'checkoutcome');      // 'OK'
